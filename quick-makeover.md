...
If you don't have a Grafana, you have two options
...
4. Under *Basemap layer*, change the Layer type to *ArcGIS MapServer* and the Server instance to *World Ocean*.
![Base layer](img/Base-layer.png)

5. Click the '&#215; Clear' text in the Search bar to clear your search for "basemap layer".

7. We want to add markers on the map.  Again using the *Search options* in the top right, find *Map layers* and click on Layer 1 *markers*.

8. We want a lookup of the country by our *geoip_country_code* field.
...
12. The colors are hard to see. We will change the tresholds:
    - Change the base color to Dark Purple by clicking on the orange circle, then clicking on dark purple, and then click outside of that popup window.
    - For a threshold of 10, change the color to dark Orange using the same method as above.
    - Delete the 3rd threshold value of 20 by clicking on its garbage can icon.
...
# Change the Dashboard ID, make the map simpler, change the last screenshot